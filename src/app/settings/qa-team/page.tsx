    ...
    const toggleMember = (user: HubstaffUser) => {
        setSelectedMembers(prev => {
            ...
            if (exists) {
                ...
                if (prev.length >= 3) {
                    alert('You can only select up to 3 QA members at a time.');
                    return prev;
                }
                return [...prev, user];
            }
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">Choose exactly 3 members for the report dropdown.</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className={`px-3 py-1 rounded-full text-sm font-bold ${selectedMembers.length === 3 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                            {selectedMembers.length} / 3 Selected
                        </span>
                        <button
                            onClick={handleSave}
