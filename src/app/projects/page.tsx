        ...
        <div className="max-w-5xl mx-auto p-6">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    ...
                    <p className="text-slate-500">Manage your project list from Hubstaff or manual entry</p>
                </div>
                <div className="flex gap-2">
                    <button
                        onClick={() => setActiveTab('import')}
                        className={`btn flex items-center gap-2 ${activeTab === 'import' ? 'bg-indigo-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}
                    >
                        <Globe size={18} /> Import from Hubstaff
                    </button>
                    <button
                        onClick={() => setActiveTab('create')}
                        className={`btn flex items-center gap-2 ${activeTab === 'create' ? 'bg-indigo-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}
                    >
                        <Plus size={18} /> Create Manual
                    </button>
                    <button
                        onClick={() => setActiveTab('list')}
                        className={`btn flex items-center gap-2 ${activeTab === 'list' ? 'bg-indigo-600 text-white' : 'bg-white text-slate-700 border border-slate-200'}`}
                    >
            ...
            {activeTab === 'list' && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                        <h3 className="font-semibold text-slate-700">All Projects ({projects.length})</h3>
                        <button onClick={fetchProjects} className="p-2 hover:bg-slate-200 rounded-lg transition-colors">
                    ...
                    {loading ? (
                        ...
                        <div className="p-12 text-center text-slate-500">No projects found in database. Import or create one.</div>
                    ) : (
                                    ...
                                    {projects.map((project) => (
                                        <tr key={project.id} className="bg-white border-b hover:bg-slate-50">
                                            ...
                                            <td className="px-6 py-4">
                                                {project.hubstaffId ? (
                                                    <span className="inline-flex items-center gap-1 text-indigo-600 bg-indigo-50 px-2 py-1 rounded-md text-xs font-medium">
                                            ...
                                            <td className="px-6 py-4 text-slate-500">{project.status}</td>
                ...
                <div className="max-w-2xl mx-auto space-y-6">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                        <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                            <Globe className="text-indigo-600" /> Import from Hubstaff
                        </h2>
                        ...
                        <div className="space-y-2">
                            {hubstaffProjects.map((hp) => {
                                ...
                                return (
                                    <div key={hp.id} className="flex items-center justify-between p-4 border border-slate-100 rounded-xl hover:bg-slate-50 transition-colors">
                                        <div>
                                            <h4 className="font-semibold text-slate-800">{hp.name}</h4>
                                        ...
                                        {isImported ? (
                                            ...
                                            <button
                                                ...
                                                className="px-4 py-2 text-sm bg-white border border-indigo-200 text-indigo-600 font-bold rounded-lg hover:bg-indigo-50 transition-colors disabled:opacity-50"
                                            >
                ...
                <div className="max-w-xl mx-auto">
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                        <h2 className="text-xl font-bold mb-6 flex items-center gap-2">
                            <Plus className="text-emerald-600" /> Create Manual Project
                        </h2>
                        <form onSubmit={createManualProject} className="space-y-4">
                            ...
                            <div className="pt-2">
                                <button
                                    type="submit"
                                    disabled={creating}
                                    className="w-full py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-colors disabled:opacity-50 flex justify-center items-center gap-2"
                                >
